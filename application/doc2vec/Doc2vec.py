...
def lac_init(model_path='', user_dict_path='', pre_alloc_size=1024 * 1024 * 16, t2s=False, just_seg=True):
    ...
    _lib.init(c_char_p(model_path), c_char_p(user_dict_path), pre_alloc_size, int(t2s), int(just_seg))  # 调用接口进行初始化
    return _lib
...
class Doc2vec(object):
    def __init__(self, save_path):
        ...
        self.lda_m.load()
        self.tfidf_m = tfidf_model(save_path + '/ldamodel/dic', save_path + '/ldamodel/tfidf')
        self.wordembedding_m = wordembedding_model(save_path + '/vec.txt.vec')
