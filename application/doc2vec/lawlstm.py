...
class lstm_model(object):
	...
	def Train(self, path):
		...
		batches = self.batch_iter(data = list(zip(x_train, y_train)), batch_size = self.config.batch_size, num_epochs = self.config.num_epochs)
			...
			with self.sess.as_default():
				...
				for batch in batches:
					...
					if current_step % 100 == 0:
						...
						print num
						for i in range(num):
							accuracy, loss = dev_step(x_dev[i * self.eval_config.batch_size:(i+1)*self.eval_config.batch_size], y_dev[i * self.eval_config.batch_size:(i+1)*self.eval_config.batch_size])
...
if __name__ == "__main__":
	...
	a.Load()
	#a.Train(path = '')
	print a.get_embedding(text = 'æˆ?çˆ?åŒ—äº¬ å¤©å®‰é—?')
